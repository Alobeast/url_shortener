...
class LinksTest < ActionDispatch::IntegrationTest
  ...
  test "links index pagination" do
    50.times { Link.create!(url: "https://ecample.org") }
    get links_path
    assert_response :ok
    assert_select "span", "<"
  end
  ...
  test "create link as guest turbo stream format" do
    assert_difference "Link.count" do
      post links_path(format: :turbo_stream), params: { link: { url: "http://google.com" } }
      assert_response :ok
      assert_nil Link.last.user_id
    end
  end

  test "create link as guest html format" do
    assert_difference "Link.count" do
      post links_path, params: { link: { url: "http://google.com" } }
